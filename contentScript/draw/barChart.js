function drawBarChart(dataset, containerId) {
  ...
  gradient.append("stop").attr("offset", "0%").attr("stop-color", "#d32f2f");

  gradient.append("stop").attr("offset", "100%").attr("stop-color", "#ff5252");
...
function reDrawBarChart(graphElements, newDataset) {
  ...
  gradient.append("stop").attr("offset", "0%").attr("stop-color", "#d32f2f"); // 금색 (yellow)

  gradient.append("stop").attr("offset", "100%").attr("stop-color", "#ff5252"); // 연한 노란색 (light yellow)
  ...
  // 막대 업데이트
  const bars = svg.selectAll("rect").data(newDataset, (d) => d.name);
  ...
  // 값 레이블 업데이트
  const labels = svg.selectAll(".value-label").data(newDataset, (d) => d.name);
